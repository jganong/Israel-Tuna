...
%% Bin SSM Positions
% some of the locations are out of bounds, and
%histcounts2 returns bin indices that are 0 for values outside the bin edges
% adding 1 would make 0 into 1 which is a good 1-based index.
% the problem is that this make the max bin 1 too big.

binned.LONedges = -6:1:40;
binned.LATedges = 30:1:46;

[binned.N,~,~,binned.indLON,binned.indLAT] = histcounts2(SSM.Longitude,SSM.Latitude,binned.LONedges,binned.LATedges);

% Create a mask for valid indices (non-zero bin assignments)
valid_mask = (binned.indLON > 0) & (binned.indLAT > 0);

% Initialize Index with NaN or 0
SSM.Index = NaN(size(binned.indLON));

% Only compute indices for valid bins
SSM.Index(valid_mask) = sub2ind(size(binned.N), ...
    binned.indLON(valid_mask), ...
    binned.indLAT(valid_mask));
...
% originally the next line read:
%binned.n(tmp.Index) = tmp.GroupCount;
% but when tmp.Index has a NaN, i get this error:
%Array indices must be positive integers or logical values.


binned.n(tmp.Index(isfinite(tmp.Index))) = tmp.GroupCount(isfinite(tmp.Index));
clear tmp
...
% originally the followig lines read:
...
binned.LONmid = diff(binned.LONedges)/2 + binned.LONedges(1);
binned.LATmid = diff(binned.LATedges)/2 + binned.LATedges(1);
